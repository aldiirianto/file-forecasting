...
//$conn = nama database
$sql2 = mysqli_query($conn, "SELECT tbl_produk.id_produk,tbl_produk.nm_produk FROM tbl_produk"); // memanggil data produk
while ($data = mysqli_fetch_array($sql2)) {
		$a = 0.1;
		$sql = mysqli_query($conn, "SELECT SUM(qty) AS qty, YEAR(date) as tahun FROM tbl_penjualan
		INNER JOIN tbl_shipment ON tbl_penjualan.id_shipment=tbl_shipment.id_shipment WHERE 
 id_produk='$data[id_produk]' GROUP BY YEAR(date) LIMIT 5"); // memanggil jumlah data penjualan berdasarkan bulan dan tahun

		$arrsementara = array(); //inisialisasi array
   		$periode = array();  //inisialisasi array
		while ($row = mysqli_fetch_array($sql)) {
			array_push($arrsementara, $row['qty']); // menginput data penjualan per bulan kedalam array untuk proses perhitungan selanjutnya
		...
		for ($loop = 0; $loop < 9; $loop++) {
			...
			$datatmp = array(); // data peramalan smoothing pertama
			...
			<h2><?php echo $data['nm_produk'] . "($a)" ?></h2>
			...
			foreach ($periode as $key => $data) {
				...
				mysqli_query($conn, "INSERT INTO tbl_peramalan VALUES ('','$data(array(id_produk))','$tmptggl','$datat','$tmp12')");
			}
